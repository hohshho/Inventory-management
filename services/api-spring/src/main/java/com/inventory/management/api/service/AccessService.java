...
public class AccessService {
    ...
    public List<ApiTypes.GroupMemberResponse> listGroupMembers(AuthenticatedUser user) {
        ...
        members.sort(
            Comparator.comparing((ApiTypes.GroupMemberResponse row) -> !"owner".equals(row.role()))
                .thenComparing(ApiTypes.GroupMemberResponse::name, String.CASE_INSENSITIVE_ORDER)
        );
        return members;
    }
    ...
    public List<ApiTypes.GroupJoinRequestResponse> listGroupJoinRequests(AuthenticatedUser user) {
        ...
        return this.groupJoinRequestRepository.findByGroupId(groupId).stream()
            .filter((GroupJoinRequestEntity request) -> "pending".equals(this.normalizeJoinRequestStatus(request.status)))
            .sorted(Comparator.comparing((GroupJoinRequestEntity request) -> request.id).reversed())
    ...
    public ApiTypes.UserSessionResponse createGroup(AuthenticatedUser user, ApiTypes.GroupRequest request) {
        ...
        if (this.groupRepository.existsByNameKeyAndCreatedByAndIsActiveTrue(nameKey, user.uid())) {
    ...
    public ApiTypes.JoinGroupResultResponse joinGroup(AuthenticatedUser user, ApiTypes.JoinGroupRequest request) {
        ...
        UserProfileEntity profile = this.requireActiveProfile(user.uid());
        Optional<GroupMembershipEntity> existingMembership = this.groupMembershipRepository.findByGroupIdAndUserId(group.id, user.uid());
        ...
        if (this.groupJoinRequestRepository.findByUserIdAndInviteCode(user.uid(), inviteCode).isPresent()) {
